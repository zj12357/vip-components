...
export class BaseValidator {
    ...
    curValidMsgTemplate: Record<string, any>;
    ...
    isRequired() {
        ...
        if (isEmptyValue(this.value) || isEmptyArray(this.value)) {
            this.error.message = [
                this.message ||
                    getMsgTemplate(this.curValidMsgTemplate, 'required', [
                        this.field,
                    ]),
            ];
    ...
    dealError(
        ...
        if (isError) {
            const theMessage =
                this.message ||
                getMsgTemplate(this.curValidMsgTemplate, errTemplate, values);
...
export class NumberValidator extends BaseValidator {
    constructor(value: any, rules: any, options: any) {
...
export class StringValidator extends BaseValidator {
    ...
    min(num: number) {
        return this.dealError(
            !isEmptyValue(this.value) && this.value.length < num,
            {
    ...
    match(regStr: string) {
        ...
        return this.dealError(
            !isEmptyValue(this.value) && !reg.test(this.value),
            {
    ...
    uppercase(isValue: boolean) {
        const isUppercase =
            (this.value || '').toLocaleUpperCase() === this.value;
        return this.dealError(
            !isEmptyValue(this.value) && isUppercase !== isValue,
            {
    ...
    lowercase(isValue: boolean) {
        const isLowercase =
            (this.value || '').toLocaleLowerCase() === this.value;

        return this.dealError(
            !isEmptyValue(this.value) && isLowercase !== isValue,
            {
    ...
    whitespace() {
        return this.dealError(
            !isEmptyValue(this.value) && isEmptyValue(this.value.trim()),
            {
    ...
    hasKeys(keys: string[]) {
        ...
        return this.dealError(
            !isEmptyValue(this.value) &&
                keys.some((theKey) => !allKeys.includes(theKey)),
            {
