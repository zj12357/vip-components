...
export const messageTemplate: Record<string, any> = {
...
export const getMsgTemplate = (
    ...
    if (temNameArr[0] in templates) {
        ...
        if (
            Object.prototype.toString.call(firstTemplate).toLowerCase() ===
                '[object object]' &&
            temNameArr.length > 1 &&
    ...
    let cur = 0;
    return theTemplate.replace(/%s/g, (curVal) =>
        cur < values.length ? values[cur++] : curVal,
    );
};

export const mergeMsgTemplate = (originMT: any, newMT: any) => {
